...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Set up the music element (optional volume setting)
  music.volume = 0.5;

  // Try playing the music automatically
  music.play().then(function() {
    // Hide the modal once the music starts playing automatically
    musicModal.style.display = 'none';
  }).catch(function(error) {
    // If autoplay fails, show modal
    console.log('Autoplay blocked, showing modal');
    musicModal.style.display = 'block'; // Show the modal asking for user interaction
  });
  ...
  startMusicButton.addEventListener('click', function() {
    if (music.paused) {
      ...
      startMusicButton.classList.add('play');
    }
  });
});
